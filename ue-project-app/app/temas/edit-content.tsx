...
export default function TemasUpdate() {
    ...
    const handleUpdateContent = async () => {
        ...
        const { error, result } = await updateContent(AuthTokenUser, setLoading, id, data);
        ...
        if (result) {
            Alert.alert("UE | Éxito", result);
            navigate(`/materias/${id}/content/details`);
        }
    }
   ...
   useEffect(() => {
    ...
    const getDetails = async () => {
        ...
        if (result) {
            setTitle(result[0].title_content)
            setDescription(result[0].description_content)
            setLevel(result[0].year_content)
        }
    }
        ...
        <ScrollView>
            ...
            >
                ...
                <ThemedView style={stylesDashboard.contentInfoUser}>
                    <ArrowBack path={`/materias/${id}/content/details`}/>
                    <ThemedText darkColor={ColorsButton.danger} lightColor={ColorsButton.danger} style={{ fontSize: 17 }}>🪔 Hey {user.name}, te encuentras actualizando <TextBrd style={{ color: ColorsButton.primary }}>{title}.</TextBrd></ThemedText>
